...
for (let x = 0; x < 10; x++) {
    ...
    let existingJSON = [];
    if (fs.existsSync(TRANSCRIPTS_OUTPUT_FILE + ".json")) {
        const data = fs.readFileSync(TRANSCRIPTS_OUTPUT_FILE + ".json");
    ...
    (async () => {
        ...
        if (existingJSON.length > 0) {
            ...
            const samuel = await JSON.parse(fs.readFileSync("1samuel.json"));
            const jeremiah = await JSON.parse(fs.readFileSync("jeremiah.json"));
            const titus = await JSON.parse(fs.readFileSync("titus.json"));
            const luke = await JSON.parse(fs.readFileSync("luke.json"));
            const thessalonians = await JSON.parse(fs.readFileSync("2thessalonians.json"));
            const specials = await JSON.parse(fs.readFileSync("specials.json"));
            const hebrews = await JSON.parse(fs.readFileSync("hebrews.json"));
            const revelation = await JSON.parse(fs.readFileSync("revelation.json"));
            const guests = await JSON.parse(fs.readFileSync("guests.json"));
        ...
        if (allTranscripts) {
            fs.writeFileSync(TRANSCRIPTS_OUTPUT_FILE + ".txt", allTranscripts);
            fs.writeFileSync(TRANSCRIPTS_OUTPUT_FILE + ".json", JSON.stringify(transcriptsJSON, null, 2));
