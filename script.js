...
const loadContents = () => {
    if (Array.isArray(results)) {
        ...
        results.forEach((el) => {
            if ((sortBy === "top" && enteredCount < loadMax) || sortBy !== "top") {
                ...
                if (prevVideoId !== el.id) {
                    ...
                    replace = true;
                    replaceID = el.id;
                    console.log("IN THE IF", el.id, replaceID);
                }
                prevVideoId = el.id;
                ...
                videoDiv += `</div></div>`;
                const emptyRep = `<div class="video"><div class="video-grid"><div class="instances"></div><div class="bracket"></div>${!window.mobileCheck() ? `<img class="thumbnail" src="https://i.ytimg.com/vi/${el.id}/mqdefault.jpg"/>` : ``}</div></div>`;
        ...
        console.log(contents.innerHTML.replace(new RegExp(regex), ''));
        ...
        console.log(new RegExp(regex).test(contents.innerHTML));
    } else {
...
window.onclick = (event) => {
    ...
    if (!event.target.matches('#upload-button') && uploadContent.style.display === "block" && !event.target.matches('#upload-content *')) {
        toggleUpload();
    }
}
...
const stateChange = () => {
    if (document.getElementById("selected-line")) {
        if (player.playerInfo.playerState === 1 && playingResponse !== "") {
            console.log("ohio");
        }
    }
}
...
function onYouTubeIframeAPIReady() {
    playerIsReady = false;
    player = new YT.Player(document.getElementById("video"), {events: {"onReady": playerReady,"onStateChange": stateChange}});
}
...
/* uploadContent.querySelectorAll("*").forEach((el) => {
    el.addEventListener("click", () => toggleUpload());
}); */
