...
if (isSafari || window.mobileCheck()) {
    ...
    document.getElementById("miniplayer").innerHTML += `<div id="resize-handle"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-bounding-box" viewBox="0 0 16 16">
...
const fetchSermons = async (VideoID = "") => {
    try {
        ...
        finalResult = finalResult.filter((el, idx) => idx !== finalResult.length - 1 ? el.date !== finalResult[idx+1].date : el.date);
        return finalResult;
    } catch (error) {
...
const loadContents = () => {
    if (Array.isArray(results)) {
            ...
            if ((sortBy === "top" && enteredCount < loadMax) || sortBy !== "top") {
                ...
                el.timestamps.forEach((instance) => {
                    ...
                    if ((sortBy === "top" && entryCount > totalLoadedContents && enteredCount < loadMax) || sortBy !== "top") {
                        ...
                        videoDiv += `
                            <div class="snippet">
                                <div onclick="miniplayerLoad('${el.id}', ${instance[0]})" class="time" style="${pH !== "00" ? "font-size: 13px;" : ""}">${timestamp}</div>
                                <div class="line">
                                    ${highlightedLine}
                                </div>
                            </div>
        ...
        const regex = `</div><div class="bracket"></div>${!window.mobileCheck() ? `<img onclick="miniplayerLoad\\('${replaceID}', 0\\)" class="thumbnail" src="https://i.ytimg.com/vi/${replaceID}/mqdefault.jpg" style="height: ${offset}px;">` : ""}</div></div>$`;
        ...
        document.querySelectorAll(".snippet").forEach((el) => {
            ...
            if (!window.mobileCheck()) el.parentElement.nextElementSibling.nextElementSibling.style.height = el.offsetHeight + "px";
        });
        ...
        if (!window.mobileCheck()) contents.style.width = contents.children[1].offsetWidth + "px";
    } else {
...
const search = async () => {
    ...
    if (keyword === "") {
        ...
        sortBy === "old" ? sortedSermons.sort((a, b) => new Date(a.date) - new Date(b.date)) : sortedSermons.sort((a, b) => new Date(b.date) - new Date(a.date));
        let sermons = [];
        ...
        document.getElementById("funkyyap").oncanplaythrough = async () => {
            ...
            setTimeout(() => document.getElementById("slideshow").src = "assets/slideshow/2&11.jpg", 2400);
            setTimeout(() => document.getElementById("slideshow").src = "assets/slideshow/3.png", 5900);
            setTimeout(() => document.getElementById("slideshow").src = "assets/slideshow/4.png", 8400);
            setTimeout(() => document.getElementById("slideshow").src = "assets/slideshow/5&10.png", 9900);
            setTimeout(() => document.getElementById("slideshow").src = "assets/slideshow/6.png", 12900);
            setTimeout(() => document.getElementById("slideshow").src = "assets/slideshow/7.png", 13900);
            setTimeout(() => document.getElementById("slideshow").src = "assets/slideshow/8.png", 15400);
            setTimeout(() => document.getElementById("slideshow").src = "assets/slideshow/9.png", 16400);
            setTimeout(() => document.getElementById("slideshow").src = "assets/slideshow/5&10.png", 18400);
            setTimeout(() => document.getElementById("slideshow").src = "assets/slideshow/2&11.jpg", 19400);
            setTimeout(() => document.getElementById("screen").style.display = "none", 21400);
        };
...
window.onclick = (event) => {
    ...
    if (window.mobileCheck() && !event.target.matches('#pastors-button') && !event.target.matches('#pastors-content') && document.getElementById("pastors-content").style.display === "block" && !event.target.matches('#pastors-content *')) {
        togglePastors();
    }
}
...
const resizeHandle = window.mobileCheck() ? document.getElementById('resize-handle') : null;
...
const miniplayerLoad = async (id, timestamp) => {
    ...
    if (!window.mobileCheck()) document.getElementById("skeleton").style.display = "none";
    video.src = `https://www.youtube.com/embed/${id}?autoplay=1&start=${fT}&enablejsapi=1`;
}

const closeMiniplayer = () => {
    document.getElementById("miniplayer").style.display = "none";
    if (!window.mobileCheck()) document.getElementById("skeleton").style.display = "block";
...
const toggleThisSermon = () => {
    ...
    if (recentSermon) document.getElementById("livestream").innerHTML = `${checkComponent} Sunday's Sermon`;
