...
const loadContents = () => {
    if (Array.isArray(results)) {
        ...
        document.querySelectorAll(".video").forEach((el) => {
            ...
            if (uploadContent.style.display === "block" || booksContent.style.display === "block" || sortContent.style.display === "block" || document.getElementById("pastors-content").style.display === "block") el.style.zIndex = "-1";
        });
...
const search = async () => {
    ...
    if (sortBy === "top" && keyword !== "") results.sort((a, b) => b.timestamps.length - a.timestamps.length);
    loadContents();
}
